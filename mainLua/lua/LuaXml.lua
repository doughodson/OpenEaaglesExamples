...
function LuaXML.tag(var,tag)
    if base.type(var)~="table" then return end
    if base.type(tag)=="nil" then 
        return var[LuaXML.TAG]
    end
    var[LuaXML.TAG] = tag
end

-- creates a new LuaXML object either by setting the metatable of an existing Lua table or by setting its tag
function LuaXML.new(arg)
    if base.type(arg)=="table" then 
        base.setmetatable(arg,{__index=xml, __tostring=LuaXML.str})
        return arg
    end
    local var={}
    base.setmetatable(var,{__index=xml, __tostring=LuaXML.str})
    if base.type(arg)=="string" then var[LuaXML.TAG]=arg end
    return var
end

-- appends a new subordinate LuaXML object to an existing one, optionally sets tag
function LuaXML.append(var,tag)
    if base.type(var)~="table" then return end
    local newVar = LuaXML.new(tag)
    var[#var+1] = newVar
    return newVar
end

-- converts any Lua var into an XML string
function LuaXML.str(var,indent,tagValue)
        ...
        for k,v in base.pairs(var) do -- attributes 
            if base.type(k)=="string" then
                if base.type(v)=="table" and k~="_M" then -- otherwise recursiveness imminent
                    tableStr = tableStr..LuaXML.str(v,indent+1,k)
                else
...
function LuaXML.save(var,filename)
...
function LuaXML.find(var, tag, attributeKey,attributeValue)
    ...
    for k,v in base.ipairs(var) do
        if base.type(v)=="table" then
            local ret = LuaXML.find(v, tag, attributeKey,attributeValue)
