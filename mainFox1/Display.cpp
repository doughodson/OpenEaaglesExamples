...
IMPLEMENT_EMPTY_SLOTTABLE_SUBCLASS(Display,"FoxDisplay")
...
//------------------------------------------------------------------------------
void Display::create(FX::FXApp* app, FX::FXComposite* const frame, FX::FXObject* tgt,
...
// toggleTranslation()
//------------------------------------------------------------------------------
void Display::toggleTranslation()
{
  translate = !translate;
}
...
//------------------------------------------------------------------------------
bool Display::setCanvasSize(const float newW, const float newH)
{
  if (glCanvas != 0) {
...
//-----------------------------------------------------------------------------
void Display::reshapeIt(int, int)
{
  if (glCanvas != 0) {
    if (!glCanvas->isCurrent()) {
      glCanvas->makeCurrent();
...
BasicGL::Graphic* Display::pick(const int mouseX, const int mouseY, const int item)
{
  GLint viewport[4];
...
//-----------------------------------------------------------------------------
void Display::clearSelectBuffer(GLuint sbuff[], const int size)
{
...
//-----------------------------------------------------------------------------
BasicGL::Graphic* Display::findSelected(const GLuint sbuff[], const int size, const int item)
{
...
void Display::printSelectBuffer(const GLuint sbuff[], const int size)
{
